...
def handler(sample):
    # Deserialize message
    compressed_mask = Mask.deserialize(sample.payload.to_bytes())
    print(f"Received message: {compressed_mask}")
...
def main():
    ...
    subscriber = session.declare_subscriber('rt/model/compressed_mask', handler)
